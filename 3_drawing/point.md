...
### 1.2 选择类型
- **作用**：选择导入数据是单个文件还是整个文件夹。
- **说明**：如果选择文件夹，软件将批量处理该文件夹下的所有点位文件。

### 1.3 XY 数据索引号
- **作用**：指定每一行数据中，哪两列分别代表 X 和 Y 坐标。
- **设置建议**：输入两位数字（如 `01`），表示第 0 列为 X，第 1 列为 Y。

### 1.4 交换 XY 数据
- **作用**：是否将 X、Y 坐标数据互换。
- **设置建议**：如果选择“是”，则第一个索引对应 Y，第二个索引对应 X。

### 1.5 比例系数
- **作用**：对 X、Y 坐标进行缩放或镜像。
- **设置建议**：输入 X 和 Y 的比例值。输入 `-1` 可实现对应轴的镜像。
